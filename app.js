// ── Pantalla completa del mapa estelar ──
    ...
    <!-- SECCIÓN: MENSAJES DE VOZ -->
    <section id="voces" class="section">
        <div class="card">
            <h2 style="font-family:'Dancing Script',cursive;font-size:2.3rem;color:var(--primary);margin-bottom:6px;">🎙️ Mis Voces para Ti</h2>
            <p style="font-style:italic;opacity:0.7;margin-bottom:20px;font-size:0.92rem;">Mensajes grabados con amor, para escuchar cuando quieras 💜</p>

            <!-- GRABAR -->
            <div style="background:linear-gradient(135deg,#f5eeff,#ffe8f8);border-radius:18px;padding:18px;margin-bottom:20px;border:1px solid rgba(138,58,185,0.15);">
                <h3 style="font-size:1rem;color:var(--primary);margin-bottom:12px;">🔴 Grabar nuevo mensaje</h3>
                <input type="text" id="voice-title" placeholder="Título del mensaje (ej: Buenos días mi amor ☀️)" style="width:100%;box-sizing:border-box;padding:10px 14px;border-radius:12px;border:1px solid rgba(138,58,185,0.25);background:#fff;font-family:inherit;font-size:0.9rem;color:var(--text);margin-bottom:12px;outline:none;">
                <div id="voice-wave-wrap" style="background:#1a0533;border-radius:14px;height:64px;display:flex;align-items:center;justify-content:center;margin-bottom:12px;overflow:hidden;position:relative;">
                    <canvas id="voice-wave" style="width:100%;height:64px;display:block;"></canvas>
                    <span id="voice-wave-idle" style="position:absolute;color:rgba(200,150,255,0.5);font-size:0.82rem;">Presiona grabar para comenzar</span>
                </div>
                <div style="text-align:center;margin-bottom:12px;">
                    <span id="voice-timer" style="font-size:1.4rem;font-weight:700;color:var(--primary);font-variant-numeric:tabular-nums;">00:00</span>
                    <span style="font-size:0.75rem;color:#a78bca;margin-left:8px;">/ máx 02:00</span>
                </div>
                <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center;margin-bottom:12px;">
                    <button id="btn-record" onclick="voiceStartRec()" style="flex:1;min-width:110px;padding:11px;border-radius:14px;border:none;background:var(--primary);color:#fff;font-weight:700;font-size:0.9rem;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;-webkit-tap-highlight-color:transparent;">🎙️ Grabar</button>
                    <button id="btn-stop-rec" onclick="voiceStopRec()" disabled style="flex:1;min-width:110px;padding:11px;border-radius:14px;border:none;background:#ccc;color:#888;font-weight:700;font-size:0.9rem;cursor:not-allowed;display:flex;align-items:center;justify-content:center;gap:6px;">⏹️ Detener</button>
                </div>
                <div id="voice-preview-wrap" style="display:none;margin-bottom:12px;">
                    <p style="font-size:0.82rem;color:var(--primary);margin-bottom:6px;font-weight:600;">🎧 Escucha antes de guardar:</p>
                    <audio id="voice-preview" controls style="width:100%;border-radius:10px;"></audio>
                </div>
                <button id="btn-save-voice" onclick="voiceSave()" style="display:none;width:100%;padding:12px;border-radius:14px;border:none;background:linear-gradient(135deg,var(--primary),var(--secondary));color:#fff;font-weight:700;font-size:0.95rem;cursor:pointer;">💾 Guardar mensaje</button>
                <div id="voice-saving-msg" style="display:none;text-align:center;color:var(--primary);font-size:0.85rem;margin-top:8px;">Guardando... ⏳</div>
            </div>

            <!-- LISTA -->
            <div>
                <h3 style="font-size:1rem;color:var(--primary);margin-bottom:14px;">💌 Mensajes guardados</h3>
                <div id="voice-list" style="display:flex;flex-direction:column;gap:12px;max-height:460px;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;padding-right:4px;">
                    <div id="voice-empty" style="text-align:center;padding:28px;color:#aaa;font-style:italic;font-size:0.88rem;">Aún no hay mensajes grabados 🎙️</div>
                </div>
            </div>
        </div>
    </section>

    <!-- MODAL LIGHTBOX PARA FOTOS Y VIDEOS EN PANTALLA COMPLETA -->
    <div id="lightbox-modal" onclick="onLightboxBgClick(event)">
        <button class="lightbox-close" onclick="closeLightbox()" title="Cerrar (Esc)">✕</button>
        <button class="lightbox-nav-btn lightbox-prev" onclick="lightboxPrev()" title="Anterior (←)">‹</button>
        <button class="lightbox-nav-btn lightbox-next" onclick="lightboxNext()" title="Siguiente (→)">›</button>
    ...
    <div id="mini-player">
        <div class="player-header">
            ...
            <button class="player-close" onclick="closeMiniPlayer()">X</button>
        </div>
        <iframe id="player-iframe" class="player-video" height="240" src="" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    </div>

    <footer>
        Con todo mi corazón para Cleidis ❤️ 2026
    </footer>
